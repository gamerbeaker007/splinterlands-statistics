...
def get_land_region_details(region):
    ...
    result = http.get(address, params=params).json()
    if result and 'data' in result:
        worksite_details = pd.DataFrame(result["data"]["worksite_details"])
        staking_details = pd.DataFrame(result["data"]["staking_details"])
        deeds = pd.DataFrame(result["data"]["deeds"])

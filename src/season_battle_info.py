...
def update_season_battle_store():
    for account in store_util.get_account_names():
        store.season_modern_battle_info_df = get_season_battles(account, store.season_modern_battle_info_df.copy(), Format.MODERN)
        store.season_wild_battle_info_df = get_season_battles(account, store.season_wild_battle_info_df.copy(), Format.WILD)

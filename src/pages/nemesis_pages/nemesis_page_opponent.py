...
layout = dbc.Row(id=nemesis_page_ids.opponent_stats)
...
def update_opponent(filtered_df):
    ...
    if not filtered_df.empty:
        ...
        if not match_type.empty:
            result_layout.append(html.H5("Battles", style={'margin-top': '20px'}))
            for index, value in match_type.items():
                ...
                result_layout.append(html.H6(str(index) + ':', style={'margin-top': '10px', 'margin-bottom': '0px'}))
                result_layout.append(html.P('Encounters: ' + str(value), style={'margin-bottom': '0px'}))
                result_layout.append(html.P('Win/Loss: ' + str(win) + '-' + str(loss),style={'margin-bottom': '0px'}))
                result_layout.append(html.P('Win % : ' + str(win/(win+loss)*100) + '%', style={'margin-bottom': '0px'}))
